...
DAALTK_DIR=`dirname $TESTS_DIR`
PYTHON_DIR=`dirname $DAALTK_DIR`

echo TESTS_DIR=$TESTS_DIR
echo DAALTK_DIR=$DAALTK_DIR
...
if [[ -e $(python2.7 -c "import daaltk") ]]; then
echo "daaltk cannot be found"
exit 1
fi
...
nosetests $TESTS_DIR --with-coverage --cover-package=daaltk --cover-erase --cover-inclusive --cover-html --with-xunit  --xunit-file=$PYTHON_DIR/nosetests.xml $EXCLUDE_OPTION
